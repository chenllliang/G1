...
from vlmgym.sandbox.games import Game2048, GameBreakout, GameMatch
from vlmgym.sandbox.MetaSandbox import SandboxMananger, GameSandbox, ExperienceQueue
...
# META_SANDBOX.random_step_sandbox([10,40,60,20,40,60,80,100])
...
def compute_format_reward_batch(responses):
    format_reward_batch = []
    # detect whether <answer> and </answer> , <think> and </think> , <perception> and </perception> exist in the response
    for response in responses:
        if "<answer>" in response and "</answer>" in response and "<think>" in response and "</think>" in response and "<perception>" in response and "</perception>" in response:
            format_reward_batch.append(1)
        else:
            format_reward_batch.append(0)
    return format_reward_batch


def compute_perception_reward_batch(perception, response):
    perception_reward_batch = []
    for perception, response in zip(perception, response):
        if perception in response:
            perception_reward_batch.append(1)
        else:
            perception_reward_batch.append(0)
    return perception_reward_batch
...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        for _ in range(self.config.trainer.total_episodes):
            for batch_dict in self.train_dataloader:
                ...
                META_SANDBOX.reset_sandbox()
                META_SANDBOX.random_step_sandbox([100]*4)


                Env_Images, Env_Prompts = META_SANDBOX.get_image_prompt_batch()
                Env_Text_Perception = META_SANDBOX.get_text_observation_batch()

                # Make a huggingface dataset from the image and prompt batch
                env_dataset_on_the_fly = make_huggingface_dataset_from_image_prompt_batch(Env_Images, Env_Prompts, Env_Text_Perception)
                rlhf_dataset_on_the_fly = RLHFDataset(
                    data_path = env_dataset_on_the_fly, 
                    tokenizer=self.tokenizer,
                    processor=self.processor,
                    prompt_key=self.config.data.prompt_key,
                    answer_key=self.config.data.answer_key,
                    ...
                    min_pixels=self.config.data.min_pixels,
                    max_pixels=self.config.data.max_pixels,
                    perception_key=self.config.data.perception_key,
                    use_groundtrurh_perception=self.config.data.use_groundtrurh_perception
                )

                # Create collated batch directly instead of using a dataloader
                env_batch_dict = collate_fn([rlhf_dataset_on_the_fly[i] for i in range(len(rlhf_dataset_on_the_fly))])
                batch = DataProto.from_single_dict(env_batch_dict)
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        # Compute the reward for each response from the sandbox
                        response_batch = self.get_response_batch(batch)

                        nested_response_batch = [[] for _ in range(self.config.worker.rollout.n)]

                        # make response_batch n lists, each inner list contains len(response_batch)/n responses
                        for i in range(len(response_batch)): 
                            nested_response_batch[i % self.config.worker.rollout.n].append(response_batch[i])

                        nested_reward_batch = META_SANDBOX.get_reward_batch_with_n_rollouts(nested_response_batch, self.config.worker.rollout.n)
                        
                        

                        # flatten the nested_reward_batch in original order
                        reward_batch = []

                        for i in range(len(nested_reward_batch[0])):
                            for j in range(self.config.worker.rollout.n):
                                reward_batch.append(game.reward_shaping(nested_reward_batch[j][i]))


                        Env_Text_Perception_Rollout = []

                        for i in range(len(response_batch)):
                            Env_Text_Perception_Rollout.append(Env_Text_Perception[i//self.config.worker.rollout.n])

                        perception_reward_batch = compute_perception_reward_batch(Env_Text_Perception_Rollout, response_batch)

                        format_reward_batch = compute_format_reward_batch(response_batch)

                        

                        # save the response batch and reward batch to a local json file, naming with the global step
                        log_path_dir = "./" + self.config.trainer.experiment_name
                        if not os.path.exists(log_path_dir):
                            os.makedirs(log_path_dir)

                        response_reward_pair = [
                            {
                                "perception": Env_Text_Perception[i//self.config.worker.rollout.n],
                                "response": response_batch[i],
                                "reward": reward_batch[i],
                                "perception_reward": perception_reward_batch[i],
                                "format_reward": format_reward_batch[i],
                                "image_index": i//self.config.worker.rollout.n
                            }
                            for i in range(len(response_batch))
                        ]

                        mean_scores = np.mean(reward_batch)
                        std_scores = np.std(reward_batch)
                        mean_perception_reward = np.mean(perception_reward_batch)
                        std_perception_reward = np.std(perception_reward_batch)
                        mean_format_reward = np.mean(format_reward_batch)
                        std_format_reward = np.std(format_reward_batch)

                        all_info = {
                            "mean_scores": mean_scores,
                            "std_scores": std_scores,
                            "mean_perception_reward": mean_perception_reward,
                            "std_perception_reward": std_perception_reward,
                            "mean_format_reward": mean_format_reward,
                            "std_format_reward": std_format_reward,
                            "response_reward_pair": response_reward_pair
                        }

                        with open(os.path.join(log_path_dir, f"response_reward_pair_{self.global_step}.json"), "w") as f:
                            json.dump(all_info, f, indent=4)

                        # print avg and std of the reward batch
                        print(f"Reward batch avg: {np.mean(reward_batch)}, std: {np.std(reward_batch)}")

                                                # save all Env_Images to a local folder named Env_Images_step_{self.global_step}
                        if not os.path.exists(os.path.join(log_path_dir, f"Env_Images_step_{self.global_step}")):
                            os.makedirs(os.path.join(log_path_dir, f"Env_Images_step_{self.global_step}"))

                        for i, image in enumerate(Env_Images):
                            image.save(os.path.join(log_path_dir, f"Env_Images_step_{self.global_step}", f"image_{i}.png"))


                        # add perception reward to the reward batch
                        final_reward_batch = []

                        for i in range(len(response_batch)):
                            final_reward_batch.append(reward_batch[i] + perception_reward_batch[i] + format_reward_batch[i])

                        reward_tensor = self.set_reward_tensor(batch, final_reward_batch)
    ...
    def get_response_batch(self, data: DataProto) -> torch.Tensor:
        response_batch = []
        ...
        for i in range(len(data)):
            ...
            response_batch.append(response_str)

        return response_batch

    def set_reward_tensor(self, data: DataProto, reward_batch: torch.Tensor):
